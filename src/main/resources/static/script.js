document.addEventListener('DOMContentLoaded', () => {
  ...
  searchButtonAi.addEventListener('click', async () => {
    ...
    const fetchSingleExercise = async (exerciseToBeUpdated) => {
      ...
      const response = await fetch('http://localhost:8080/psyba/api/exercise/singleExercise', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      editBtn.addEventListener('click', () => {
        ...
        searchBtn.addEventListener('click', async () => {
          ...
          try {
                      ...
                      const response = await fetch(`http://localhost:8080/psyba/api/exercise?${params.toString()}`, {
                        method: 'GET'
                      });
